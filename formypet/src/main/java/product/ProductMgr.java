...
public class ProductMgr {
	...
	public ArrayList<ProductBean> getBathList(){
		...
		try {
			...
			sql = "select * from product where categoryKey=1 order by productOrderCount desc limit 3";
	...
	public ArrayList<ProductBean> getSnackList(){
		...
		try {
			...
			con = pool.getConnection();
			//주문 수대로 내림차순 정렬하고 위에서 3개 불러옴
			sql = "select * from product where categoryKey=6 order by productOrderCount desc limit 3";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
		...
		public ArrayList<ProductBean> getLivingList(){
			...
			try {
				...
				con = pool.getConnection();
				//주문 수대로 내림차순 정렬하고 위에서 3개 불러옴
				sql = "select * from product where categoryKey=4 order by productOrderCount desc limit 3";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
		...
		public ArrayList<ProductBean> getClothesList(){
			...
			try {
				...
				con = pool.getConnection();
				//주문 수대로 내림차순 정렬하고 위에서 3개 불러옴
				sql = "select * from product where categoryKey=7 order by productOrderCount desc limit 3";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
		...
		public ArrayList<ProductBean> getWalkList(){
			...
			try {
				...
				con = pool.getConnection();
				//주문 수대로 내림차순 정렬하고 위에서 3개 불러옴
				sql = "select * from product where categoryKey=5 order by productOrderCount desc limit 3";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
